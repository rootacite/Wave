...
public partial class GameScripting
{
    ...
    public HWaveController CreateHWave(Vector2 Pos, List<CirculKey> Childrens, float Length, float LastTime, float Offset = 1, float Scale = 1, GameObject Parent = null)
    {
        float max_z = 0;
        ...
        var _hwave = HWaveController.Creat(this, new Vector3(Pos.x, Pos.y, max_z + 0.01f), HWave, Parent == null ? KeyLayer : Parent,Length, SecondPerBeat, Childrens, LastTime, Offset, Scale);
        #region Behavior
        ...
        (Vector3 Head, Vector3 End) DrawDragRoute(CirculKey i)
        {
            ...
            {
                var StartPos = OriginPos.Offset((float)i.IDragData.From, (float)(_hwave.RealRod * (i.WaveOffset / Length)));
                List<Polar2> KFrameList = new List<Polar2>();


                foreach (var z in i.IDragData.DragRoute)
                {
                    KFrameList.Add(new Polar2(Polar2.d2r(z.d¦È), _hwave.RealRod * ((i.WaveOffset + z.¦Ñ) / Length)));
                }
                PolarSystem.EnumPolarRoute((p, ii) =>
                {
                    Vector3 ccp = p.ToVector();
        ...
        void DrawSingleRoute(KeyRoute Route/**/)
        {
            for (int v = 0; v < Route.Count; v++)
            {
                ...
                foreach (var i in Route[v])
                {
                    ...
                    PrePoint.Create(Pos.Offset(i.Angle, _hwave.RealRod * ((float)i.WaveOffset / Length)), PrePoint_Obj, KeyLayer, (float)(i.WaveOffset + Offset) * SecondPerBeat, 1 / (SecondPerBeat * HeadPending), i.Type);
                }
